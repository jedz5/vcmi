...
namespace {
	...
	int distToFarestNeighbour(BattleHex hex, const ReachabilityInfo::TDistances& dists, BattleHex *chosenHex = nullptr)
	{
		int ret = 0;
		for (BattleHex n : hex.neighbouringTiles())
		{
			if (dists[n] >= 0 && dists[n] < GameConstants::BFIELD_SIZE && dists[n] > ret)
			{
				ret = dists[n];
				if (chosenHex)
					*chosenHex = n;
	...
	try
	{
		...
		if (targets.possibleAttacks.size())
		{
			...
			if (hlp.attack.shooting)
				if (!stack->moved() && !stack->waited() && hlp.enemy->position.getX() > 7 && !hlp.enemy->moved() && !hlp.enemy->getCreature()->isShooting())
				{
				...
				PotentialTargets enemyTargets(hlp.enemy);
				if (enemyTargets.possibleAttacks.size())
				{
		...
		else
		{
			if (stack->waited())
			{
				auto enemy =  cb->battleGetStacksIf([=](const CStack * s) {
					return s->isValidTarget(false) && !s->attackerOwned && !s->getCreature()->isShooting();
				});
				if (enemy.size())
					return goNorthWest();
...
void CGeniusAI::battleStacksAttacked(const std::vector<BattleStackAttacked> & bsa)
{
...
void CGeniusAI::battleNewRoundFirst(int round)
{
...
void CGeniusAI::battleStackMoved(const CStack * stack, std::vector<BattleHex> dest, int distance)
{
...
void CGeniusAI::battleStacksEffectsSet(const SetStackEffect & sse)
{
...
void CGeniusAI::battleStacksHealedRes(const std::vector<std::pair<ui32, ui32> > & healedStacks, bool lifeDrain, bool tentHeal, si32 lifeDrainFrom)
{
	print("battleStacksHealedRes called");
}

void CGeniusAI::battleNewStackAppeared(const CStack * stack)
{
	print("battleNewStackAppeared called");
}

void CGeniusAI::battleObstaclesRemoved(const std::set<si32> & removedObstacles)
{
	print("battleObstaclesRemoved called");
}

void CGeniusAI::battleCatapultAttacked(const CatapultAttack & ca)
{
	print("battleCatapultAttacked called");
}

void CGeniusAI::battleStacksRemoved(const BattleStacksRemoved & bsr)
{
...
{
	logAi->trace("CGeniusAI [%p]: %s", this, text);
}

BattleAction CGeniusAI::goTowards(const CStack * stack, BattleHex destination)
{
	assert(destination.isValid());
	if (stack->position == destination) // already in place
		return BattleAction::makeDefend(stack);
	...
	auto destNeighbours = destination.neighbouringTiles();
	if (vstd::contains_if(destNeighbours, [&](BattleHex n) { return stack->coversPos(n); }))
	{
	...
	else
	{
		...
		if (distToFarestNeighbour(destination, reachability.distances, &bestNeighbor) > GameConstants::BFIELD_SIZE)
		{
...
std::vector<BattleHex> CGeniusAI::getTargetsToConsider(const CSpell * spell, const ISpellCaster * caster) const
{
